...
$(document).ready(function () {
	...
	$('#schedule-tabs a').click(function (e) {
        ...
        $('#office-hours-cont').mouseenter(function(){
                $('#office-hours-open').show(1);
                $('#office-hours-closed').hide(1);
        });

        $('#office-hours-cont').mouseleave(function(){
                $('#office-hours-open').hide(1);
                $('#office-hours-closed').show(1);
        });
	...
	// Countdown //
	$('#countdown').countdown({
		until: new Date(2015, 9 - 1, 24), // new Date(year, mth - 1, day, hr, min, sec) - date/time to count down to 
	...
	// Google Map //
	$('#map_canvas').gmap({
		'center': new google.maps.LatLng(44.64261, -63.57871), // Change this to your desired latitude and longitude
		'zoom': 17,
	...
	var image = {
		url: 'images/mapmarker.png', // Define the map marker file here
	...
	$('#map_canvas').gmap().bind('init', function () {
		$('#map_canvas').gmap('addMarker', {
			'id': 'marker-1',
			'position': '44.64261, -63.57871',
			'bounds': false,
			'icon': image
		}).click(function () {
			$('#map_canvas').gmap('openInfoWindow', {
				'content': '<h4>Startup Empire</h4><p><strong>Park Lane Theatre Downtown Halifax</strong><br>5657 Spring Garden Rd, Halifax, NS B3J 3R4 </p>'
			}, this);
		});
	});
